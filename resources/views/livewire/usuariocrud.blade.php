<div class="py-10"
  ...
  <div class="container mx-auto px-4">
    ...
    <div class="bg-[#181818] rounded-lg shadow-lg p-4 mb-6 border border-black/40 text-white">
      <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div class="w-full md:max-w-md">
          <label class="block text-sm font-medium text-gray-300 mb-1">Buscar Usuario</label>
          <div class="flex gap-2">
            <input wire:keydown.enter="clickBuscar()"
                   type="search"
                   wire:model="search"
    ...
    {{-- TÍTULO --}}
    <h2 class="text-5xl font-extrabold text-center mb-4 text-[#ff7a00] drop-shadow-md">USUARIOS</h2>
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 text-gray-900">
              <a href="mailto:{{$item->email}}" class="text-[#ff7a00] hover:text-[#ff9d2e] hover:underline">
                {{$item->email}}
              </a>
            </td>
            <td class="px-6 py-4">
    ...
    <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999]">
      <div class="bg-[#181818] rounded-xl shadow-2xl w-full max-w-lg p-6 border border-black/60 text-white">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-[#ff7a00] font-bold text-lg">Datos del Usuario</h2>
        ...
        <form class="space-y-4">
          ...
          <div>
            ...
            <input wire:model="apellidos" type="text"
                   class="w-full px-3 py-2 border rounded bg-[#2a2a2a] text-white
                          focus:outline-none focus:ring-2 focus:ring-[#ff7a00] border-black/60"
                   required>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-200">Nombre Usuario</label>
            <input wire:model="nombre_usuario" type="text"
                   class="w-full px-3 py-2 border rounded bg-[#2a2a2a] text-white
                          focus:outline-none focus:ring-2 focus:ring-[#ff7a00] border-black/60"
                   required>
            @error('nombre_usuario') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-200">Contraseña</label>
            <input wire:model="contrasena" type="password"
                   class="w-full px-3 py-2 border rounded bg-[#2a2a2a] text-white
                          focus:outline-none focus:ring-2 focus:ring-[#ff7a00] border-black/60"
                   required>
            @error('contrasena') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-200">Tipo</label>
            ...
            @error('tipo') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
          </div>
