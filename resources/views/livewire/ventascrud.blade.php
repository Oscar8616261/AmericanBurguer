...
<div class="bg-gray-100 p-4">
    <div class="grid grid-cols-4 gap-4">
        ...
        <div class="bg-white p-4 rounded-lg shadow-md col-span-1">
            ...
            @if(auth()->guard('web')->check())
                <h2 class="text-lg font-semibold mb-3">Datos del Cliente</h2>
            @elseif(auth()->guard('clientes')->check())
                <h2 class="text-lg font-semibold mb-3">Escoja un operador de venta</h2>
            @else
                <h2 class="text-lg font-semibold mb-3">Inicia sesión para continuar</h2>
            @endif
            ...
            @if(auth()->guard('web')->check())
                ...
                <button wire:click="buscarCliente"
                    class="bg-orange-500 hover:bg-orange-600 text-white w-full py-2 rounded mb-2">
                    Buscar Cliente
                </button>
                ...
                {{-- Datos del Cliente (solo si existe un cliente encontrado) --}}
                @if($clienteId)
        ...
        <div class="col-span-3 space-y-4">
            ...
            <div class="bg-white p-4 rounded-lg shadow-md">
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @forelse ($categorias as $index => $item)
                        <div class="flex items-center rounded-lg shadow cursor-pointer hover:scale-105 transition duration-200 p-2"
                             style="background-color: {{ ['#ffeb3b', '#ff9800', '#8bc34a', '#03a9f4', '#e91e63'][$index % 5] }};"
                             wire:click="prodcutosCategoria({{ $item->id_categoria }})">
            ...
            <!-- Buscador -->
            <div class="bg-white p-4 rounded-lg shadow-md">
                <h2 class="text-lg font-semibold mb-2">Buscar Producto</h2>
                <div class="flex items-center space-x-2">
                    <input type="search" wire:model="searchProducto" wire:keydown.enter="clickBuscar"
                        placeholder="Buscar Producto"
                        class="w-full max-w-md px-4 py-2 text-sm border rounded-lg bg-gray-200 focus:outline-none"/>
                    <button wire:click="clickBuscar"
                        class="bg-[hsl(25,95%,53%)] hover:opacity-90 rounded-lg px-4 py-2 text-white">
                        Buscar
                    </button>
                </div>
            </div>
            ...
            <!-- Lista de Productos -->
            <div class="bg-white grid grid-cols-2 gap-4 rounded-lg shadow-md p-4">
                @forelse ($productos as $item)
                    <div class="flex items-center space-x-4 p-3 border rounded-lg">
                        <div class="shrink-0">
                            <img class="w-16 h-16 rounded-lg object-cover" src="/storage/img/{{$item->foto}}" alt="Producto">
                        </div>
                        ...
                        <div>
                            <button wire:click="addProducto({{$item->id_producto}})" class="bg-green-600 hover:bg-green-700 w-10 h-10 rounded text-white">
    ...
    <div class="grid grid-cols-3 gap-4 mt-4">
        ...
        <div class="bg-white p-4 rounded-lg shadow-md col-span-2">
            <h2 class="text-lg font-semibold mb-3 text-center">Carrito de Compras</h2>
            ...
            @forelse ($carrito as $item)
                <div class="flex justify-between items-center border-b py-2">
                    ...
                    <div class="flex space-x-1">
                        <button wire:click="addProducto({{$item['producto']['id_producto']}})" class="bg-green-600 hover:bg-green-700 w-8 h-8 rounded text-white">+</button>
                        <button wire:click="removeProducto({{$item['producto']['id_producto']}})" class="bg-red-600 hover:bg-red-700 w-8 h-8 rounded text-white">-</button>
                    </div>
                </div>
            @empty
        ...
        <div class="bg-white p-4 rounded-lg shadow-md">
            ...
            {{-- Botón cambia según el tipo de usuario --}}
