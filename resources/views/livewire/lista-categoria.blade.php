<div>
    ...
    <div class="flex ...">
        <div class="flex-auto w-32 ...">
            <input  wire:keydown.enter="clickBuscar()" type="search" wire:model="search" class="w-full max-w-md px-4 py-2 text-sm text-[#000000] border border-[#37383a] rounded-lg bg-[#d1d4da] focus:ring-[#c70606] focus:border-[#c70606] " placeholder="Buscar Categoria" />
    ...
    <div class="bg-[#e8e8eb] py-10">
        ...
        <div class="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="w-full max-w-sm bg-gray-100 border border-gray-200 rounded-lg shadow-sm p-4">
                <img src="/storage/img/{{$item->foto}}" class="w-full h-48 object-cover rounded-lg mb-4">
                
                <h3 class="font-bold text-xl mb-2 text-center">{{$item->nombre}}</h3>
                <p class="text-gray-600 text-center">{{$item->descripcion}}</p>  
            
                <!-- Botones de Editar y Eliminar -->
                @auth('web')
                <div class="flex justify-between mt-4">
                    <button wire:click.prevent="editar({{$item->id_categoria}})" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 w-1/2 mr-2">
                        Editar
                    </button>
                    <button wire:click.prevent="delete({{$item->id_categoria}})" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 w-1/2 ml-2">
                        Eliminar
                    </button>
                </div>
                @endauth
        ...
        <div class="fixed inset-0 bg-[#9b9b9b2d] bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-[#ffffff] p-6 rounded-lg w-96">
                <div class="flex justify-between items-center">
                    <h2 class="text-xl font-semibold">Detalles de la Categoría</h2>
